...
const RacketComparison: React.FC<{ language: Language }> = ({ language }) => {
  ...
  const filteredRackets = useMemo(() => {
    ...
    return list;
  }, [dbRackets, query, selectedBrand]);

  const handleSearch = async (searchQuery?: string) => {
    const finalQuery = searchQuery || query;
    if (!finalQuery.trim()) return;
    try {
      const result = await compare_rackets(finalQuery, language);
      setDbRackets(prev => {
        const combined = [...prev];
        result.rackets.forEach(nr => { 
          if (!combined.some(pr => pr.name.toLowerCase() === nr.name.toLowerCase())) {
            combined.push({ ...nr, id: crypto.randomUUID() });
          }
        });
        return combined;
      });
