...
export const fetchTennisNews = async (lang: Language): Promise<{ articles: any[] }> => {
  ...
  const response = await fetchWithRetry<GenerateContentResponse>(() => ai.models.generateContent({
    ...
    contents: `Get 5 top stories in professional tennis. Title, summary, source, URL. Output JSON in ${lang}.`,
    config: { 
      systemInstruction: "Tennis news aggregator. Quick JSON output. Use Google Search. No extra text.",
      tools: [{ googleSearch: {} }],
      responseMimeType: "application/json",
      responseSchema: {
        type: Type.OBJECT,
  ...
  try {
    const rawText = response.text?.trim() || '{"articles": []}';
  ...
  const response = await fetchWithRetry<GenerateContentResponse>(() => ai.models.generateContent({
    ...
    config: { 
      imageConfig: { 
        ...
        ...(model === 'gemini-3-pro-image-preview' ? { imageSize: size as any } : {})
      } 
    }
  }));
  for (const part of response.candidates?.[0]?.content?.parts || []) {
...
export const compare_rackets = async (query: string, lang: Language): Promise<{ rackets: RacketSpec[] }> => {
  ...
  power level (1-10), control level (1-10), comfort level (1-10), player type, recommended level, and a brief technical summary. 
        ...
        properties: {
          rackets: {
            ...
            items: {
              ...
              required: ['name', 'brand', 'weight', 'headSize', 'powerLevel', 'controlLevel', 'comfortLevel']
            }
          }
        },
        required: ['rackets']
