...
import { activateUserAccount } from './services/payment/stripepayment';
...
const App: React.FC = () => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        try {
          ...
          if (user) saveAnalysisToDb(user.uid, result);
        } catch (err: any) {
  ...
  useEffect(() => {
    ...
    console.log("Checking for payment success param:", paymentSuccess);
    console.log("Checking for payment canceled param:", paymentCanceled);

    if (paymentSuccess === 'true') {
      if (user) {
        console.log("User returned from payment. Optimistically showing success view.");
        // Não tentamos escrever no Firestore aqui para evitar erros de permissão.
        // O AuthProvider já está escutando as mudanças (via Stripe Extension).
        setToast({ message: state.language === 'pt' ? 'Pagamento Recebido! Processando...' : 'Payment Received! Processing...', visible: true });
        window.history.replaceState({}, document.title, window.location.pathname);
        setState(p => ({ ...p, view: 'success' }));
      } else {
        console.log("Payment success param present, but NO USER logged in.");
      }
    } else if (paymentCanceled === 'true') {
      setToast({ message: state.language === 'pt' ? 'Pagamento Cancelado' : 'Payment Canceled', visible: true });
      window.history.replaceState({}, document.title, window.location.pathname);
      setState(p => ({ ...p, view: 'canceled' }));
    }
  }, [user, state.language]);
  ...
  const isTrialPage = ['rackets', 'torneios', 'courts'].includes(state.view);
  const hasAccess = userData?.status === 'active' || ['news', 'success', 'canceled'].includes(state.view) || (isTrialPage && isTrialActive);
  ...
  return (
    <div className="min-h-screen-ios flex flex-col bg-[#232323] text-white overflow-x-hidden">
        ...
        <div key={state.view} className="animate-in fade-in slide-in-from-bottom-5 h-full min-h-[50vh]">
          {!hasAccess ? (
            ...
            <>
              ...
              {state.view === 'success' && <Success onBack={() => setState(p => ({ ...p, view: 'judge' }))} />}
              {state.view === 'canceled' && <Canceled onBack={() => setState(p => ({ ...p, view: 'judge' }))} />}
            </>
          )}
        </div>
      </main>
      ...
      <nav className="fixed bottom-0 left-0 right-0 lg:hidden bg-[#111111]/95 backdrop-blur-2xl border-t border-white/5 flex flex-col items-center safe-bottom pt-4 z-[200] shadow-[0_-15px_40px_rgba(0,0,0,0.8)]">
        <div className="w-full flex justify-around items-end px-2 min-h-[5.5rem] pb-4">
        ...
        {/* Trial Countdown in Footer */}
        {isTrialActive && (
          <div className="w-full px-4 mb-4 flex justify-center">
            <TrialCountdown isPt={state.language === 'pt'} />
          </div>
        )}
      </nav>
    </div>
